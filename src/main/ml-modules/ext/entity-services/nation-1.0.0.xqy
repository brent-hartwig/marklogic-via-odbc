...
(:
 ...
 The source model was nation-1.0.0
 ...
 Generated at timestamp: 2020-11-23T21:40:37.121386Z
 :)

module namespace nation
    = 'http://example.org/nation-1.0.0';
...
(:~
 ...
 :   data for populating a nation
 : @return A map:map instance with extracted data and
 :   metadata about the instance.
 :)
declare function nation:extract-instance-nation(
    $source as item()?
) as map:map
{
    let $source-node := es:init-source($source, 'nation')
    let $NATIONKEY  :=             $source-node/NATIONKEY ! xs:int(.)
    let $NAME  :=             $source-node/NAME ! xs:string(.)
    let $REGIONKEY  :=             $source-node/REGIONKEY ! xs:int(.)
    let $COMMENT  :=             $source-node/COMMENT ! xs:string(.) 
    let $instance := es:init-instance($source-node, 'nation')
    ...
    then $instance
        ...
        =>   map:with('NATIONKEY', $NATIONKEY)
        =>es:optional('NAME', $NAME)
        =>es:optional('REGIONKEY', $REGIONKEY)
...
(:~
 ...
 :)
declare function nation:instance-to-canonical(
...
) as node()
{
        ...
        then xdmp:to-json( nation:canonicalize($entity-instance) )/node()
        else nation:instance-to-canonical-xml($entity-instance)
};
...
(:~
 ...
 :)
declare private function nation:canonicalize(
    $entity-instance as map:map
) as map:map
{
    json:object()
                    ...
                    return
                        ...
                        case json:object
                            return
                                ...
                                else map:put($m, $key, nation:canonicalize($instance-property))
                        ...
                        case json:array
                            return
                                ...
                                return
                                    ...
                                    then json:set-item-at($instance-property, $i, nation:canonicalize($val))
                                    else (),
...
(:~
 ...
 : @return An XML element that encodes the instance.
 :)
declare private function nation:instance-to-canonical-xml(
    $entity-instance as map:map
) as element()
{
                ...
                return
                    ...
                    case json:object+
                        return
                            ...
                            return element { $ns-key } { nation:instance-to-canonical-xml($prop) }
                    (: An array can also treated as multiple elements :)
                    case json:array
                        return
                            ...
                            return
                                ...
                                then element { $ns-key } {
                                    ...
                                    nation:instance-to-canonical-xml($val)
                                }
                                else element { $ns-key } {
                                    attribute datatype { 'array' },
                                    $val }
...
(:
 ...
 :)
declare function nation:instance-to-envelope(
    $entity-instance as map:map,
    $envelope-format as xs:string
) as document-node()
{
    let $canonical := nation:instance-to-canonical($entity-instance, $envelope-format)
...
{
    nation:instance-to-envelope($entity-instance, "xml")
};
