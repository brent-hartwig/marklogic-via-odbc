...
(:
 ...
 The source model was partsupp-1.0.0
 ...
 Generated at timestamp: 2020-11-23T21:40:37.353788Z
 :)

module namespace partsupp
    = 'http://example.org/partsupp-1.0.0';
...
(:~
 ...
 :   data for populating a partsupp
 : @return A map:map instance with extracted data and
 :   metadata about the instance.
 :)
declare function partsupp:extract-instance-partsupp(
    $source as item()?
) as map:map
{
    let $source-node := es:init-source($source, 'partsupp')
    let $PARTKEY  :=             $source-node/PARTKEY ! xs:int(.)
    let $SUPPKEY  :=             $source-node/SUPPKEY ! xs:int(.)
    let $AVAILQTY  :=             $source-node/AVAILQTY ! xs:int(.)
    let $SUPPLYCOST  :=             $source-node/SUPPLYCOST ! xs:decimal(.)
    let $COMMENT  :=             $source-node/COMMENT ! xs:string(.) 
    let $instance := es:init-instance($source-node, 'partsupp')
    ...
    then $instance
        ...
        =>es:optional('SUPPKEY', $SUPPKEY)
        =>es:optional('AVAILQTY', $AVAILQTY)
        =>es:optional('SUPPLYCOST', $SUPPLYCOST)
...
(:~
 ...
 :)
declare function partsupp:instance-to-canonical(
...
) as node()
{
        ...
        then xdmp:to-json( partsupp:canonicalize($entity-instance) )/node()
        else partsupp:instance-to-canonical-xml($entity-instance)
};
...
(:~
 ...
 :)
declare private function partsupp:canonicalize(
    $entity-instance as map:map
) as map:map
{
    json:object()
                    ...
                    return
                        ...
                        case json:object
                            return
                                ...
                                else map:put($m, $key, partsupp:canonicalize($instance-property))
                        ...
                        case json:array
                            return
                                ...
                                return
                                    ...
                                    then json:set-item-at($instance-property, $i, partsupp:canonicalize($val))
                                    else (),
...
(:~
 ...
 : @return An XML element that encodes the instance.
 :)
declare private function partsupp:instance-to-canonical-xml(
    $entity-instance as map:map
) as element()
{
                ...
                return
                    ...
                    case json:object+
                        return
                            ...
                            return element { $ns-key } { partsupp:instance-to-canonical-xml($prop) }
                    (: An array can also treated as multiple elements :)
                    case json:array
                        return
                            ...
                            return
                                ...
                                then element { $ns-key } {
                                    ...
                                    partsupp:instance-to-canonical-xml($val)
                                }
                                else element { $ns-key } {
                                    attribute datatype { 'array' },
                                    $val }
...
(:
 ...
 :)
declare function partsupp:instance-to-envelope(
    $entity-instance as map:map,
    $envelope-format as xs:string
) as document-node()
{
    let $canonical := partsupp:instance-to-canonical($entity-instance, $envelope-format)
...
{
    partsupp:instance-to-envelope($entity-instance, "xml")
};
