...
(:
 ...
 The source model was region-1.0.0
 ...
 Generated at timestamp: 2020-11-23T21:40:37.414855Z
 :)

module namespace region
    = 'http://example.org/region-1.0.0';
...
(:~
 ...
 :   data for populating a region
 : @return A map:map instance with extracted data and
 :   metadata about the instance.
 :)
declare function region:extract-instance-region(
    $source as item()?
) as map:map
{
    let $source-node := es:init-source($source, 'region')
    let $REGIONKEY  :=             $source-node/REGIONKEY ! xs:int(.)
    let $NAME  :=             $source-node/NAME ! xs:string(.)
    let $COMMENT  :=             $source-node/COMMENT ! xs:string(.) 
    let $instance := es:init-instance($source-node, 'region')
...
(:~
 ...
 :)
declare function region:instance-to-canonical(
...
) as node()
{
        ...
        then xdmp:to-json( region:canonicalize($entity-instance) )/node()
        else region:instance-to-canonical-xml($entity-instance)
};
...
(:~
 ...
 :)
declare private function region:canonicalize(
    $entity-instance as map:map
) as map:map
{
    json:object()
                    ...
                    return
                        ...
                        case json:object
                            return
                                ...
                                else map:put($m, $key, region:canonicalize($instance-property))
                        ...
                        case json:array
                            return
                                ...
                                return
                                    ...
                                    then json:set-item-at($instance-property, $i, region:canonicalize($val))
                                    else (),
...
(:~
 ...
 : @return An XML element that encodes the instance.
 :)
declare private function region:instance-to-canonical-xml(
    $entity-instance as map:map
) as element()
{
                ...
                return
                    ...
                    case json:object+
                        return
                            ...
                            return element { $ns-key } { region:instance-to-canonical-xml($prop) }
                    (: An array can also treated as multiple elements :)
                    case json:array
                        return
                            ...
                            return
                                ...
                                then element { $ns-key } {
                                    ...
                                    region:instance-to-canonical-xml($val)
                                }
                                else element { $ns-key } {
                                    attribute datatype { 'array' },
                                    $val }
...
(:
 ...
 :)
declare function region:instance-to-envelope(
    $entity-instance as map:map,
    $envelope-format as xs:string
) as document-node()
{
    let $canonical := region:instance-to-canonical($entity-instance, $envelope-format)
...
{
    region:instance-to-envelope($entity-instance, "xml")
};
