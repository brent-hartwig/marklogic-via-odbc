...
(:
 ...
 The source model was orders-1.0.0
 ...
 Generated at timestamp: 2020-11-23T21:40:37.186953Z
 :)

module namespace orders
    = 'http://example.org/orders-1.0.0';
...
(:~
 ...
 :   data for populating a orders
 : @return A map:map instance with extracted data and
 :   metadata about the instance.
 :)
declare function orders:extract-instance-orders(
    $source as item()?
) as map:map
{
    let $source-node := es:init-source($source, 'orders')
    let $ORDERKEY  :=             $source-node/ORDERKEY ! xs:int(.)
    let $CUSTKEY  :=             $source-node/CUSTKEY ! xs:int(.)
    let $ORDERSTATUS  :=             $source-node/ORDERSTATUS ! xs:string(.)
    let $TOTALPRICE  :=             $source-node/TOTALPRICE ! xs:decimal(.)
    let $ORDERDATE  :=             $source-node/ORDERDATE ! xs:date(.)
    let $ORDERPRIORITY  :=              $source-node/ORDERPRIORITY ! xs:string(.)
    let $CLERK  :=             $source-node/CLERK ! xs:string(.)
    let $SHIPPRIORITY  :=             $source-node/SHIPPRIORITY ! xs:int(.)
    let $COMMENT  :=             $source-node/COMMENT ! xs:string(.) 
    let $instance := es:init-instance($source-node, 'orders')
    ...
    then $instance
        ...
        =>es:optional('CUSTKEY', $CUSTKEY)
        =>es:optional('ORDERSTATUS', $ORDERSTATUS)
        =>es:optional('TOTALPRICE', $TOTALPRICE)
        =>es:optional('ORDERDATE', $ORDERDATE)
        =>es:optional('ORDERPRIORITY', $ORDERPRIORITY)
        =>es:optional('CLERK', $CLERK)
        =>es:optional('SHIPPRIORITY', $SHIPPRIORITY)
...
(:~
 ...
 :)
declare function orders:instance-to-canonical(
...
) as node()
{
        ...
        then xdmp:to-json( orders:canonicalize($entity-instance) )/node()
        else orders:instance-to-canonical-xml($entity-instance)
};
...
(:~
 ...
 :)
declare private function orders:canonicalize(
    $entity-instance as map:map
) as map:map
{
    json:object()
                    ...
                    return
                        ...
                        case json:object
                            return
                                ...
                                else map:put($m, $key, orders:canonicalize($instance-property))
                        ...
                        case json:array
                            return
                                ...
                                return
                                    ...
                                    then json:set-item-at($instance-property, $i, orders:canonicalize($val))
                                    else (),
...
(:~
 ...
 : @return An XML element that encodes the instance.
 :)
declare private function orders:instance-to-canonical-xml(
    $entity-instance as map:map
) as element()
{
                ...
                return
                    ...
                    case json:object+
                        return
                            ...
                            return element { $ns-key } { orders:instance-to-canonical-xml($prop) }
                    (: An array can also treated as multiple elements :)
                    case json:array
                        return
                            ...
                            return
                                ...
                                then element { $ns-key } {
                                    ...
                                    orders:instance-to-canonical-xml($val)
                                }
                                else element { $ns-key } {
                                    attribute datatype { 'array' },
                                    $val }
...
(:
 ...
 :)
declare function orders:instance-to-envelope(
    $entity-instance as map:map,
    $envelope-format as xs:string
) as document-node()
{
    let $canonical := orders:instance-to-canonical($entity-instance, $envelope-format)
...
{
    orders:instance-to-envelope($entity-instance, "xml")
};
