...
(:
 ...
 The source model was part-1.0.0
 ...
 Generated at timestamp: 2020-11-23T21:40:37.281134Z
 :)

module namespace part
    = 'http://example.org/part-1.0.0';
...
(:~
 ...
 :)
declare function part:extract-instance-part(
    $source as item()?
) as map:map
{
    ...
    let $MFGR  :=             $source-node/MFGR ! xs:string(.)
    let $BRAND  :=             $source-node/BRAND ! xs:string(.)
    let $TYPE  :=             $source-node/TYPE ! xs:string(.)
    let $SIZE  :=             $source-node/SIZE ! xs:int(.)
    let $CONTAINER  :=             $source-node/CONTAINER ! xs:string(.)
    let $RETAILPRICE  :=             $source-node/RETAILPRICE ! xs:decimal(.)
    ...
    then $instance
        ...
        =>es:optional('MFGR', $MFGR)
        =>es:optional('BRAND', $BRAND)
        =>es:optional('TYPE', $TYPE)
        =>es:optional('SIZE', $SIZE)
        =>es:optional('CONTAINER', $CONTAINER)
        =>es:optional('RETAILPRICE', $RETAILPRICE)
...
(:~
 ...
 :)
declare function part:instance-to-canonical(
...
) as node()
{
        ...
        then xdmp:to-json( part:canonicalize($entity-instance) )/node()
        else part:instance-to-canonical-xml($entity-instance)
};
...
(:~
 ...
 :)
declare private function part:canonicalize(
    $entity-instance as map:map
) as map:map
{
    json:object()
                    ...
                    return
                        ...
                        case json:object
                            return
                                ...
                                else map:put($m, $key, part:canonicalize($instance-property))
                        ...
                        case json:array
                            return
                                ...
                                return
                                    ...
                                    then json:set-item-at($instance-property, $i, part:canonicalize($val))
                                    else (),
...
(:~
 ...
 : @return An XML element that encodes the instance.
 :)
declare private function part:instance-to-canonical-xml(
    $entity-instance as map:map
) as element()
{
                ...
                return
                    ...
                    case json:object+
                        return
                            ...
                            return element { $ns-key } { part:instance-to-canonical-xml($prop) }
                    (: An array can also treated as multiple elements :)
                    case json:array
                        return
                            ...
                            return
                                ...
                                then element { $ns-key } {
                                    ...
                                    part:instance-to-canonical-xml($val)
                                }
                                else element { $ns-key } {
                                    attribute datatype { 'array' },
                                    $val }
...
(:
 ...
 :)
declare function part:instance-to-envelope(
    $entity-instance as map:map,
    $envelope-format as xs:string
) as document-node()
{
    let $canonical := part:instance-to-canonical($entity-instance, $envelope-format)
...
{
    part:instance-to-envelope($entity-instance, "xml")
};
