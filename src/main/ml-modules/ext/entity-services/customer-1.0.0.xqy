...
(:
 ...
 The source model was customer-1.0.0
 ...
 Generated at timestamp: 2020-11-23T15:29:44.619985Z
 :)

module namespace customer
    = 'http://example.org/customer-1.0.0';
...
(:~
 ...
 :)
declare function customer:extract-instance-customer(
    $source as item()?
) as map:map
{
    ...
    let $NATIONKEY  :=             $source-node/NATIONKEY ! xs:int(.)
    let $PHONE  :=             $source-node/PHONE ! xs:string(.)
    let $ACCTBAL  :=             $source-node/ACCTBAL ! xs:decimal(.)
    let $MKTSEGMENT  :=             $source-node/MKTSEGMENT ! xs:string(.)
    ...
    then $instance
        ...
        =>   map:with('CUSTKEY', $CUSTKEY)
        =>es:optional('NAME', $NAME)
        =>es:optional('ADDRESS', $ADDRESS)
        =>es:optional('NATIONKEY', $NATIONKEY)
        =>es:optional('PHONE', $PHONE)
        =>es:optional('ACCTBAL', $ACCTBAL)
        =>es:optional('MKTSEGMENT', $MKTSEGMENT)
...
(:~
 ...
 :)
declare function customer:instance-to-canonical(
...
) as node()
{
        ...
        then xdmp:to-json( customer:canonicalize($entity-instance) )/node()
        else customer:instance-to-canonical-xml($entity-instance)
};
...
(:~
 ...
 :)
declare private function customer:canonicalize(
    $entity-instance as map:map
) as map:map
{
    json:object()
                    ...
                    return
                        ...
                        case json:object
                            return
                                ...
                                else map:put($m, $key, customer:canonicalize($instance-property))
                        ...
                        case json:array
                            return
                                ...
                                return
                                    ...
                                    then json:set-item-at($instance-property, $i, customer:canonicalize($val))
                                    else (),
...
(:~
 ...
 : @return An XML element that encodes the instance.
 :)
declare private function customer:instance-to-canonical-xml(
    $entity-instance as map:map
) as element()
{
                ...
                return
                    ...
                    case json:object+
                        return
                            ...
                            return element { $ns-key } { customer:instance-to-canonical-xml($prop) }
                    (: An array can also treated as multiple elements :)
                    case json:array
                        return
                            ...
                            return
                                ...
                                then element { $ns-key } {
                                    ...
                                    customer:instance-to-canonical-xml($val)
                                }
                                else element { $ns-key } {
                                    attribute datatype { 'array' },
                                    $val }
...
(:
 ...
 :)
declare function customer:instance-to-envelope(
    $entity-instance as map:map,
    $envelope-format as xs:string
) as document-node()
{
    let $canonical := customer:instance-to-canonical($entity-instance, $envelope-format)
...
{
    customer:instance-to-envelope($entity-instance, "xml")
};
